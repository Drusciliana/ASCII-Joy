...
---------------------------------------------------------------------------------------------------
-- desc: Default configuration tables.
---------------------------------------------------------------------------------------------------
...
tick = 0;
LastZone = 9999; -- To remember when we are zoning.
mb_data = {};
arraySize = 0;
local HeartNum = {};
local heart = {};
...
ashita.events.register('load', 'load_cb', function ()
    ...
    update_settings(s); -- Hopefully loads correct settings.
...
-- Monster Window labels
    local m = AshitaCore:GetFontManager():Create('__asciijoy_mobhpp');
    ...
    m:SetVisible(false);
    local n = AshitaCore:GetFontManager():Create('__asciijoy_mobinfo');
    ...
    n:SetVisible(false);
    local o = AshitaCore:GetFontManager():Create('__asciijoy_mobweak');
    ...
    o:SetVisible(false);
    local p = AshitaCore:GetFontManager():Create('__asciijoy_mobsub');
...
---- Self Window labels
    local q = AshitaCore:GetFontManager():Create( '__asciijoy_selfhp' );
    q:SetBold(true);
    ...
    local r = AshitaCore:GetFontManager():Create( '__asciijoy_petname' );
    r:SetBold(true);
    ...
    local s1 = AshitaCore:GetFontManager():Create( '__asciijoy_pethp' );
    s1:SetBold(true);
    s1:SetColor(0xFFFFFFFF);
    s1:SetFontFamily(ascii_config.selffont.name);
    s1:SetFontHeight(ascii_config.selffont.size);
    s1:SetPositionX(0);
    s1:SetPositionY(0);
    s1:SetText('PET HP');
    s1:SetLocked(true);
    s1:SetVisible(false);
    s1:GetBackground():SetColor(Background);
    s1:GetBackground():SetVisible(true);
    local t = AshitaCore:GetFontManager():Create( '__asciijoy_petinfo' );
    t:SetBold(true);
    ...
    local u = AshitaCore:GetFontManager():Create( '__asciijoy_selfnum' );
    u:SetBold(true);
...
---- Party Window labels
    local e = AshitaCore:GetFontManager():Create('__asciijoy_partytitle');
...
--    for x = 0, 17 do  -- Not ready do try alliance yet
    for x = 0, 5 do
        f = AshitaCore:GetFontManager():Create(string.format('%s%d', '__asciijoy_partyhpp', x));
        f:SetColor(0xFFFFFFFF);
        f:SetFontFamily(ascii_config.partyfont.name);
        f:SetFontHeight(ascii_config.partyfont.size);
        f:SetBold(true);
        f:SetRightJustified(true);
        f:SetPositionX(0);
        f:SetPositionY(0);
        f:SetText(string.format('XXX %d', x));
        f:SetLocked(true);
  	f:GetBackground():SetColor(Background);
        f:GetBackground():SetVisible(true);
        f:SetVisible(false);
	g = AshitaCore:GetFontManager():Create(string.format('%s%d', '__asciijoy_partympp', x));
        g:SetColor(0xFFFFFFFF);
        g:SetFontFamily(ascii_config.partyfont.name);
        g:SetFontHeight(ascii_config.partyfont.size);
        g:SetBold(true);
        g:SetRightJustified(true);
        g:SetPositionX(0);
        g:SetPositionY(0);
        g:SetText(string.format('YYY %d', x));
        g:SetLocked(true);
        g:GetBackground():SetColor(Background);
        g:GetBackground():SetVisible(true);
        g:SetVisible(false);
	h = AshitaCore:GetFontManager():Create(string.format('%s%d', '__asciijoy_partyspc', x));
        h:SetColor(0xFFFFFFFF);
        h:SetFontFamily(ascii_config.partyfont.name);
        h:SetFontHeight(ascii_config.partyfont.size);
        h:SetBold(true);
        h:SetRightJustified(true);
        h:SetPositionX(0);
        h:SetPositionY(0);
        h:SetText(string.format('ZZZ %d', x));
        h:SetLocked(true);
        h:GetBackground():SetColor(Background);
        h:GetBackground():SetVisible(true);
        h:SetVisible(false);
    end

-- Get Info and what not for Mobs upon loading the addon.
    ZoneIDStart = AshitaCore:GetMemoryManager():GetParty():GetMemberZone(0);
    local _, mb_data = pcall(require,'data.'..tostring(ZoneIDStart));
...
ashita.events.register('unload', 'unload_cb', function ()
    -- Cleanup the font objects and get rid of the default. Again, for good measure..
    local character_path = settings.settings_path();
    local defaults_path = ('%s\\config\\addons\\%s\\%s\\'):fmt(AshitaCore:GetInstallPath(), addon.name, 'defaults');
    if (character_path == defaults_path) then -- Both paths identical. Ugh.
	return;
...
ashita.events.register('d3d_present', 'present_cb', function ()
    ...
    local e = AshitaCore:GetFontManager():Get('__asciijoy_partytitle');
    local m = AshitaCore:GetFontManager():Get('__asciijoy_mobhpp');
    local n = AshitaCore:GetFontManager():Get('__asciijoy_mobinfo');
    local o = AshitaCore:GetFontManager():Get('__asciijoy_mobweak');
    local p = AshitaCore:GetFontManager():Get('__asciijoy_mobsub');
    local q = AshitaCore:GetFontManager():Get('__asciijoy_selfhp');
    local r = AshitaCore:GetFontManager():Get('__asciijoy_petname');
    local s1 = AshitaCore:GetFontManager():Get('__asciijoy_pethp');
    local t = AshitaCore:GetFontManager():Get('__asciijoy_petinfo');
    local u = AshitaCore:GetFontManager():Get('__asciijoy_selfnum');

    for x = 1, 5 do
	heart[x] = AshitaCore:GetPrimitiveManager():Get(string.format('%s%d', '__asciijoy_heart', x));
    end

    for x = 1, 12 do
	HeartContainer[x] = AshitaCore:GetPrimitiveManager():Get(string.format('%s%d', '__asciijoy_container', x));
    end
   ...
   -- Don't show the party window if we're alone? Also need that playerfound for making entities for personal stats. Ugh.
	...
	for x = 0, 5 do --** May be different for alliance
          f = AshitaCore:GetFontManager():Get(string.format('%s%d', '__asciijoy_partyhpp', x));
	  g = AshitaCore:GetFontManager():Get(string.format('%s%d', '__asciijoy_partympp', x));
	  h = AshitaCore:GetFontManager():Get(string.format('%s%d', '__asciijoy_partyspc', x));
	  f:SetVisible(false);
	  g:SetVisible(false);
	  h:SetVisible(false);
	end
  ...
  if (ascii_config.options.party == true) then
    ...
    -------- Get Party Mana, TP, Class
	    ...
	    local ManaStr = '';
            ...
            g:SetPositionX(e:GetPositionX());
            g:SetPositionY(newy);
	    g:SetBold(true);
            h:SetPositionX(e:GetPositionX());
            h:SetPositionY(spcy);
	    h:SetBold(true);

    ---- Get Party Members' Job(s)
	    MJob = jobs[party:GetMemberMainJob(x)];
	    if (party:GetMemberSubJobLevel(x) > 0) then
    ...
    ---- Final Window Output
		...
		OutThr = string.sub(OutThr, 1, 35);
	 	OutThr = " "..OutThr.." ";
        	...
        	if (HPValue <= 33) then
            	    if (tick >= 15) then
                        f:GetBackground():SetColor(0x5fff0000);
                    else
                	f:GetBackground():SetColor(0x5F000000);
                    end
                else
            	    e:GetBackground():SetColor(0x5F000000);
		end
	        ...
	        if (elsewhere == false) then      			-- Try to put in Zone Name for far away friends
	            ...
	            f:SetText(tostring(Output));
	            g:SetText(tostring(OutTwo));
	        else 
...
-------- Player Window
    if(ascii_config.options.self == true) then
...
------------- HEART STUFF
	if(ascii_config.options.zilda == true) then
		...
		sResult = ' LIFE ';
            ...
            s1:SetPositionX(q:GetPositionX());
	    s1:SetPositionY(q:GetPositionY() + ((selffontsize * 4) + 2 + 27));
	    ...
	    q:SetFontHeight(10);
            s1:SetPositionX(q:GetPositionX());
	    s1:SetPositionY(q:GetPositionY() + ((selffontsize * 4) + 2));
            t:SetPositionX(q:GetPositionX());
...
---- PET STUFF
	    ...
	    s1:SetText(string.format(pResult));
...
ashita.events.register('command', 'command_cb', function (ee)
	...
	if(ascii_config.options.self == true) then
	    ascii_config.options.self = false;
	    print(chat.header(addon.name):append(chat.message('Player Window DISABLED.')));
	elseif(ascii_config.options.self == false) then
	    ascii_config.options.self = true;
