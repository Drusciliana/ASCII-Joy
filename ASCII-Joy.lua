...
-------- Monster Window
   ...
   ---- If the target has a sub ID, use that and refence that instead.
 	    ...
 	    if (spawn ~= 16) then     -- Differentiate Monsters from NPC's/Players/Goblin Footprints/etc.
	        ...
	        OutFou = mobResult;
			local test = get_target_ht();
			local testname = '';
			local testtargeted = tarmob.TargetedIndex;
			local testaction = target.ActionTargetServerId;
			local testinter = tarmob.InteractionTargetIndex;
			if (test ~= nil) then
				testname = 'Yep';
			else
				testname = 'Nope';
			end
            OutFiv = MobName..'  -- '..testname..' '..tostring(testaction)..'   '..tostring(testtargeted)..'  '..tostring(testinter);
	        OutSix = MobWeak;
